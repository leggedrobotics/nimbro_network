...
Optional:
 ...
 - `topic_prefix` (string): prepend topic_prefix before subscibing to specified topics `name` (TCP only, default is empty string). This is a feature to allow to prefix topics names with a string on the sender side while keeping the topic name specified in `name` on the receiver side.
